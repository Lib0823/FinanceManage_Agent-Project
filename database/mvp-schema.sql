...
COMMENT ON TABLE user_kis_accounts IS '사용자별 한국투자증권 API 키 및 계좌번호';
COMMENT ON COLUMN user_kis_accounts.app_key IS 'KIS APP Key';
COMMENT ON COLUMN user_kis_accounts.app_secret IS 'KIS APP Secret';
...
CREATE INDEX idx_trade_history_date ON trade_history(executed_at);

COMMENT ON TABLE trade_history IS '자동매매 체결 이력';
COMMENT ON COLUMN trade_history.order_type IS '주문 유형 (buy: 매수, sell: 매도)';
COMMENT ON COLUMN trade_history.quantity IS '체결 수량';
COMMENT ON COLUMN trade_history.price IS '체결 단가';
COMMENT ON COLUMN trade_history.executed_at IS '체결 시각';
COMMENT ON COLUMN trade_history.kis_order_no IS 'KIS API 주문번호';
